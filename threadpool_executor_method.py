...
class Mathematician:
    def power_of_2(self, arg=-1):
        ...
        print('Finished for: %s: %d' % (threading.current_thread().name, arg ** 2))


def main():
    math = Mathematician()
    executor = ThreadPoolExecutor(max_workers=5)
    task1 = executor.submit(math.power_of_2)  # we can skip arguments
    task2 = executor.submit(math.power_of_2, 2)
    task3 = executor.submit(math.power_of_2, 3)
